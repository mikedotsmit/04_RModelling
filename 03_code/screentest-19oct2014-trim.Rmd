...
```
```{r }
write.table(FeedRawPSD, here("03_code/01_code-output","FeedRawPSD.txt"), sep=",", quote = FALSE,row.names = FALSE)
write.table(OSRawPSD, here("03_code/01_code-output","OSRawPSD.txt"), sep=",", quote = FALSE, row.names = FALSE)
write.table(USRawPSD, here("03_code/01_code-output","USRawPSD.txt"), sep=",", quote = FALSE, row.names = FALSE)
...
FeedRawPSD2 <- read_csv(here("03_code/01_code-output", "FeedRawPSD.csv"), col_names = TRUE) #just calling variable #2 as it was brought in, and not to overwrite the existing variable in the RSudio environment.
OSRawPSD2 <- read_csv(here("03_code/01_code-output", "OSRawPSD.csv"), col_names = TRUE)
USRawPSD2 <- read_csv(here("03_code/01_code-output", "USRawPSD.csv"), col_names = TRUE)
...
##
transformedFeed <- long_FeedRawPSD2 %>% 
  group_by (stream)%>% 
...
Coef_RRFeed <- tidy_reg_RRFeed %>% 
  dplyr::select(stream,term, estimate)

Coef_GGSFeed <- tidy_reg_GGSFeed %>% 
  dplyr::select(stream,term, estimate)

#OS coef
  
Coef_RROS <- tidy_reg_RROS %>% 
  dplyr::select(stream,term, estimate)


Coef_GGSOS <- tidy_reg_GGSOS %>% 
  dplyr::select(stream,term, estimate)

#US Coef

Coef_RRUS <- tidy_reg_RRUS %>% 
  dplyr::select(stream,term, estimate)

Coef_GGSUS <- tidy_reg_GGSUS %>% 
  dplyr::select(stream,term, estimate)



#write the tibbles to \01_code-output folder  for use in table generated in resultsanddiscion section
write.table(Coef_RRFeed, file = here("03_code/01_code-output","CoefRRFeed.txt"), sep=",", quote = FALSE)
write.table(Coef_GGSFeed, file = here("03_code/01_code-output","CoefGGSFeed.txt"), sep=",", quote = FALSE)

write.table(Coef_RROS, file = here("03_code/01_code-output","CoefRROS.txt"), sep=",", quote = FALSE)
write.table(Coef_GGSOS, file = here("03_code/01_code-output","CoefGGSOS.txt"), sep=",", quote = FALSE)


write.table(Coef_RRUS, file = here("03_code/01_code-output","CoefRRUS.txt"), sep=",", quote = FALSE)
write.table(Coef_GGSUS, file = here("03_code/01_code-output","CoefGGSUS.txt"), sep=",", quote = FALSE)
...
write.csv(R2_RR, here("03_code/01_code-output","R2RR.csv"),row.names = FALSE)
write.csv(R2_GGS, here("03_code/01_code-output","R2GGS.csv"),row.names = FALSE)
...
fitFeed2 <- regressionFeed %>%
  ...
  select(stream,RRx,RRy,GGSx,GGSy)  
...
plotRR <- ggplot(fit2, aes(x = RRx, y = RRy, color = stream) ) +
     geom_point() +
     geom_smooth(method = "lm", se = FALSE) #se is confidense intervals

plotGGS <- ggplot(fit2, aes(x = GGSx, y = GGSy, color = stream) ) +
     geom_point() +
     geom_smooth(method = "lm", se = FALSE) #se is confidense intervals
